...
class TradePlayer(object):
    ...
    def run(self, game):
        player1_data = game.memory.read_bytes(
            self.player1.address, offsets["Base"]["Player Offset Length"]
        )
        player2_data = game.memory.read_bytes(
            self.player2.address, offsets["Base"]["Player Offset Length"]
        )
        game.memory.write_bytes(
            self.player1.address,
            player2_data,
