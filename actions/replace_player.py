...
class ReplacePlayer(object):
    ...
    def run(self):
        if self.new_player == True:
            new_player_data = self.game.memory.read_bytes(self.new_player.address, offsets["Base"]["Player Offset Length"])
            self.game.memory.write_bytes(self.old_player.address, new_player_data, offsets["Base"]["Player Offset Length"])