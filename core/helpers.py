...
def ConvertToGameValue(integer, length):
    ...
    """
    game_value = (integer - 25) * 3  # Scale the value to the game's range (0-255 aka. the 8-bit range)
    game_value = max(0, min(game_value, 255)) # Clamp the value to ensure it stays within the 0-255 range
